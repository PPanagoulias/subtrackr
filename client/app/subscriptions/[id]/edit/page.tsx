    ...
    function () {
      async function fetchData() {
        try {
          ...
          const subscriptionResponse: SubscriptionResponse = await apiRequest(
          ...
          setWebsiteUrl(subscription.websiteUrl || "");
          setNotes(subscription.notes || "");
          setAutoRenew(subscription.autoRenew);
          setReminderDaysBefore(String(subscription.reminderDaysBefore || 3));

          setManagementUrl(subscription.managementUrl || "");
          setCancelInstructions(subscription.cancelInstructions || "");
          setPlannedCancelDate(
            formatDateForInput(subscription.plannedCancelDate)
          );
        } catch (err) {
